...
/**
 * Cloud Function: Translate with Tone
 *
 * Translates a message with tone awareness using RAG from Pinecone
 * Retrieves sender's previous messages to understand their communication style
 *
 * @param data.text - Text to translate
 * @param data.sourceLang - Source language (BCP 47)
 * @param data.targetLang - Target language (BCP 47)
 * @param data.chatId - Chat ID (for RAG context)
 * @param data.senderId - Sender ID (for RAG context)
 * @returns Translation with detected tone
 */
export const translateWithTone = functions.https.onCall(async (request) => {
  // 1. Check authentication
  if (!request.auth) {
    throw new functions.https.HttpsError(
      'unauthenticated',
  ...
  // 3. Validate input
  const { text, sourceLang, targetLang, chatId, senderId } = request.data;
  ...
  // Skip translation if same base language
  ...
  try {
    console.log(
      `🌐 Translating with tone: "${text.substring(0, 50)}..." from ${sourceLang} to ${targetLang}`
    );
    console.time('translation-with-tone');

    // 4. Get RAG context from Pinecone (if chatId and senderId provided)
    ...
    // 5. Build system prompt with or without context
...
Format your response as JSON:
{
  ...
  "tone": "brief description of emotional tone (1-2 words)"
    ...
    console.timeEnd('translation-with-tone');
    console.log(`✅ Translation complete: "${translatedText.substring(0, 50)}..." (tone: ${tone})`);
